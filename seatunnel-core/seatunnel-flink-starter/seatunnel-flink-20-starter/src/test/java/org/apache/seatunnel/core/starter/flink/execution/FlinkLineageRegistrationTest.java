 ...
 * API.
 *
 * <p>The starter compiles against Flink 1.15, which has no {@code JobStatusHook}, so the same
 * assertions in {@code seatunnel-flink-starter-common} could only ever be skipped. This module puts
 * Flink 1.20 on the test classpath, which is the only place the hook and the result listener are
 * reachable at all.
 */
class FlinkLineageRegistrationTest {
    ...
    void keepsFlinkCallbacksBestEffortWhenTheLineageBackendFails() throws Exception {
        ...
        FlinkLineageSupport.Registration registration =
                Assertions.assertDoesNotThrow(() -> register(environment));
        ...
        Object hook = registeredHook(registration);
    ...
    void handsTheTerminalEventToTheClientForAnAttachedSubmission() throws Exception {
        FlinkLineageSupport.Registration registration = register(environmentWithOneSink());

        Object handler = Proxy.getInvocationHandler(registeredHook(registration));
        Assertions.assertTrue(handler instanceof LineageJobStatusHook);
        Field field = LineageJobStatusHook.class.getDeclaredField("clientReportsCompletion");
        field.setAccessible(true);
        Assertions.assertTrue(
                field.getBoolean(handler),
    ...
    private static FlinkLineageSupport.Registration register(
            StreamExecutionEnvironment environment) {
    ...
    private static Object registeredHook(FlinkLineageSupport.Registration registration)
            throws Exception {
